...
bool Crosswalk::ApplyRule(Frame *frame,
  ...
  for (const auto* path_obstacle : path_decision->path_obstacles().Items()) {
    ...
    for (auto crosswalk_overlap : crosswalk_overlaps_) {
      ...
      bool stop = false;
      if (obstacle_l_distance >= FLAGS_crosswalk_max_l2_distance_to_ignore_pedestrian) {
        ...
        // (2) when l_distance is <=l1 + on_road(not on sideway), always STOP
        // (3) when l_distance is <=l1 + not on_road(on sideway), STOP only if path crosses
        if (on_road || (not on_road && is_cross)) {
          stop = true;
...
bool Crosswalk::FindCrosswalks(
  ...
  const std::vector<hdmap::PathOverlap> &crosswalk_overlaps = reference_line_info
